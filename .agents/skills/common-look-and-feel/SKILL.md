...
5. `references/asia-allied-baseline.md` explains provenance, evidence classes and usage limits.
6. `templates/admin-cms.html` is a static composition reference for admin/CMS screens only.
...
3. **Load the normative contract.** Read root `DESIGN.md`; use generated `design-system/theme.css`, `design-system/tailwind.theme.json`, `design-system/tokens.json` or `design-system/components.json` for the target stack.
...
5. **Map semantics, not raw hex values.** Use semantic names such as `primary`, `accent-accessible`, `text-muted`, `table-header` and `focus`. Brand orange is decorative; use the accessible orange token for normal-size white text.
...
10. **Run repository gates.** Run the narrow behavior test, then `make check`. Record only checks that actually passed.
